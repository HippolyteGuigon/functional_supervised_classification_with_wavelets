...
\end{itemize}
Sur le plan empirique, la règle d'usage fait jeu égal avec la règle de l'énergie sur les données de parole et sur les ECG dans le domaine temporel. Elle est meilleure sur les périodogrammes d'ECG et, surtout, beaucoup plus robuste : une seule courbe aberrante sur cinquante dégrade fortement la règle de l'énergie et laisse la règle d'usage intacte. Elle est en revanche moins performante sur les données simulées de l'article en petit échantillon, où l'information discriminante est portée par des coefficients rares ; cet écart disparaît quand la taille d'échantillon augmente.

La règle d'usage n'est donc pas une règle « plus performante » : elle définit une représentation commune plus stable, plus robuste et plus facile à interpréter, au prix d'une perte d'efficacité lorsque la classe dépend d'un détail rare. C'est le compromis visé.
